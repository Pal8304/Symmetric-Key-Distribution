...
class DiffieHellman(object):
	def __init__(self , session_id , generator=2, group=17, key_length=540):
        ...
        # The function getPrime() gives a prime number in hexadecimal format according to rfc-3526
		self.prime = self.get_prime_number(group)

		self.private_key = self.generate_private_key(key_length)
		self.public_key = self.generate_public_key()

	def get_prime_number(self, group=17):
		default_group = 17
	...
	def generate_private_key(self, bits):
		return self.generate_random(bits)
	...
	def generate_secret(self, private_key, other_key):
		if(self.check_public_key(other_key) == True):
			shared_secret = pow(other_key, private_key, self.prime)
			return shared_secret
		else:
			raise Exception("Invalid public key.")

	def generate_key(self, other_key):
		shared_secret = self.generate_secret(self.private_key, other_key)
		
		shared_secret_bytes = str(shared_secret).encode()
		session_id_bytes = str(self.session_id).encode()
		combined_data = shared_secret_bytes + session_id_bytes
		
		final_key_hash = hashlib.sha256(combined_data).hexdigest()
		self.key =  final_key_hash
...
if __name__=="__main__":
	...
	a.generate_key(b.public_key)
	b.generate_key(a.public_key)
	...
	if(a.get_key() == b.get_key()):
		...
		print("Shared secret A: ", a.get_key())
		print("Shared secret B: ", b.get_key())